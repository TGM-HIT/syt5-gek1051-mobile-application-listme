...
```
Nutzer tippt "Milch" ein
    |
    v
Vue-Component updated sofort (optimistic UI)
    |
    v
CRDT-Operation erzeugt: {type: "item.add", data: {name: "Milch"}, clock: {deviceA: 6}}
    |
    +---> IndexedDB (Dexie) - lokal gespeichert
    |
    +---> Online? --yes--> WebSocket (STOMP) ---> Server
    |         |                                      |
    |         no                                     v
    |         |                              CRDT merge + DB persist
    |         v                                      |
    |    SyncQueue (flush bei reconnect)              v
    |                                         Broadcast an alle
    |                                         anderen Geräte
    v
UI zeigt "Milch" sofort an (egal ob online/offline)
```
...
```
Device A: POST /api/lists/{id}/share
    -> generiert 12-stelligen Token (z.B. "aB3xKm9pQr2w")
    -> gibt Share-Link zurück

Jemand öffnet den Link:
    GET /api/share/aB3xKm9pQr2w
    -> zeigt Listenname + Preview

    POST /api/share/aB3xKm9pQr2w/join (mit eigenem X-Device-Id)
    -> Gerät wird als "editor" zur Liste hinzugefügt
```
...
Wenn jemand seine Listen vom Handy auf den Laptop übertragen will:

```
Handy: POST /api/sync
    -> erzeugt 24-stelligen Token (30 Tage gültig)
    -> Token bündelt ALLE Listen des Geräts

Laptop: POST /api/sync/{token}/apply (mit neuem X-Device-Id)
    -> Laptop wird zu allen Listen hinzugefügt
    -> Laptop sieht ab jetzt dieselben Listen
```
...
- **Backend:** JUnit 5 + TestContainers (PostgreSQL in Docker). Integrationstests decken den kompletten Flow ab (Device -> List -> Share -> Sync).
- **Frontend:** Vitest (Unit) + Playwright (E2E mit Offline-Simulation)
- **CRDT-Tests:** Property-Based Tests für Kommutativität, Assoziativität, Idempotenz
